...
import (
	...
	"goskeleton/app/http/controller/dns"
...
type DnsRegister struct {
	...
	DnsContent   string `form:"dns_content" json:"dns_content"  binding:"required,min=1"` // 必填、对于文本,表示它的长度>=1
	SingleRecord bool   `form:"single_record" json:"single_record"`
}

// 特别注意: 表单参数验证器结构体的函数，绝对不能绑定在指针上
// 我们这部分代码项目启动后会加载到容器，如果绑定在指针，一次请求之后，会造成容器中的代码段被污染

func (r DnsRegister) CheckParams(context *gin.Context) {
	...
	if extraAddBindDataContext == nil {
		response.ErrorSystem(context, "DnsRegister表单验证器json化失败", "")
	} else {
		// 验证完成，调用控制器,并将验证器成员(字段)递给控制器，保持上下文数据一致性
		(&dns.DnsRegister{}).Register(extraAddBindDataContext)
	}
